...
def stream_subprocess_output(input_data, placeholder, chat_placeholder):
    ...
    for line in iter(process.stdout.readline, ''):
        ...
        final_result += line
        chat_placeholder.text(final_result)
...
if st.button('Solve'):
    if api_key and programming_problem:
        ...
        streaming_thread = threading.Thread(
            ...
            args=(input_data, stream_placeholder, chat_placeholder),
            daemon=True
        )
        streaming_thread.start()

        # Display a loading message while the subprocess is running
        with st.spinner('Solving...'):
            while streaming_thread.is_alive():
                # Update the Streamlit app every second while waiting
                time.sleep(1)

        # Retrieve the final output from the thread
        final_result = stream_subprocess_output(input_data, stream_placeholder, chat_placeholder)
